...
home = '/home/testuser/.gnupg'#marionette_tg.conf.get("crypt.clienthomedir")
...
def send(channel, marionette_state, input_args, blocking=True):
    ...
    if stream_id or blocking:
      ...
      #  ptxt = str(ptxt)
       # ptxt = b64encode(ptxt)
       # ptxt = str(ptxt)
        ptxt = gpg.encrypt(ptxt, '304EA0C8E16234376720D23467B0FA789F5866E1')#str(serverkey))
       # ptxt = fteObj.encode(ptxt)#1
        ...
        #ptxt = fteObj.encode(ptxt)#1        
       # ptxt = str(ptxt)#1
        #ptxt = scrypt.encrypt(ptxt, password, maxtime=0.1)#1
       # ptxt = str(ptxt)#1
       # ptxt = b64encode#1
    ...
    try:
        ...
        if len(ctxt) > 0:
           ...
           # ctxt = str(ctxt)
            ...
            #ctxt = fteObj.decode(ctxt)#1
            ctxt = str(ctxt)
            ctxt = gpg.decrypt(ctxt, passphrase='testpassword')
           ...
           # ctxt = b64decode(ctxt)#1
            ...
            #ctxt = fteObj.decode(ctxt)#1
            #ctxt = str(ctxt)#1            
            #ctxt = scrypt.decrypt(ctxt, password, maxtime=0.1)#1
            #ctxt = str(ctxt)#1 
            #ctxt = b64decode(ctxt)#1
